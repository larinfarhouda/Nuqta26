-- Add vendor_slug to get_events_pro return type
-- First drop both overloads, then recreate with vendor_slug

-- Drop the version WITH p_country parameter
drop function if exists get_events_pro(float, float, float, text, float, float, text, timestamptz, timestamptz, int, int, text);

-- Drop the version WITHOUT p_country parameter (may not exist, safe to try)
drop function if exists get_events_pro(float, float, float, text, float, float, text, timestamptz, timestamptz, int, int);

-- Recreate with vendor_slug included and p_country parameter
...
returns table (
  ...
  vendor_name text, vendor_logo text, vendor_slug text, slug text
)
language plpgsql as $$
begin
  return query
