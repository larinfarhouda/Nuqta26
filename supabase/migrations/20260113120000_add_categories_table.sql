...
-- Public read access
drop policy if exists "Categories publicly viewable" on categories;
...
-- Drop legacy check constraint that limits event_type values
...
-- Make category_id required (optional, but good practice if all events must have a category)
-- We'll leave it nullable for a moment just in case, or enforce it if data migration is guaranteed.
-- let's enforce it for published events maybe? For now, we update the check constraint.

-- 4. Update get_events_pro to return category details
-- We need to drop and recreate the function to change return type and logic
...
create or replace function get_events_pro(
  ...
  p_category text default null, -- This will now accept SLUG (for backward compat) or ID ideally. Let's keep supporting SLUG.
...
begin
  ...
  where
    e.status = 'published'
    and (p_category is null or c.slug = p_category or e.event_type = p_category) -- Support filtering by slug
  ...
  having
    (p_min_price is null or coalesce(min((select t.price from tickets t where t.event_id = e.id limit 1)), 0) >= p_min_price) -- Simplified check (note: previous query had subquery in select, here we might want to optimize but this is fine for now)
