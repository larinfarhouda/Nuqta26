...
-- Update get_events_pro to calculate price from tickets
...
returns table (
  ...
  description text,
  image_url text,
  price numeric, -- calculated min price
  date timestamptz,
  status text,
  event_type text,
  capacity int,
...
begin
  ...
  left join tickets t on t.event_id = e.id -- Join for filtering
  where
    e.status = 'published'
  ...
  group by e.id, v.id -- Group by to handle multiple tickets join if we needed it for filtering, but for price calculation in SELECT subquery is cleaner. 
                      -- However, for price FILTERING (p_min_price), we need access to the calculated price or join.
                      -- Let's use HAVING for price filtering to avoid complex subqueries in WHERE or making the query too slow.
  having 
    (p_min_price is null or coalesce(min(t.price), 0) >= p_min_price)
    and (p_max_price is null or coalesce(min(t.price), 0) <= p_max_price)
